...
#define FLAG_IDL_REGISTER                                                      \
  0x001000                        /* generates a registration (_r.res) file    \
                                   */
#define FLAG_IDL_TYPELIB 0x002000 /* generates a typelib (_l.res) file */
#define FLAG_IDL_REGTYPELIB                                                    \
  0x004000 /* generates a registered typelib (_t.res) file */
...
static const struct {
  ...
  const char *ext;
    ...
    {FLAG_IDL_TYPELIB, "_l.res"},  {FLAG_IDL_REGTYPELIB, "_t.res"},
    {FLAG_IDL_CLIENT, "_c.c"},     {FLAG_IDL_IDENT, "_i.c"},
    {FLAG_IDL_PROXY, "_p.c"},      {FLAG_IDL_SERVER, "_s.c"},
...
static const char *install_targets[NB_INSTALL_RULES] = {
    "install-lib", "install-dev", "install-test"};
static const char *install_variables[NB_INSTALL_RULES] = {
...
static void fatal_error(const char *msg, ...) {
  ...
  if (input_file_name) {
    ...
    if (input_line)
      fprintf(stderr, "%d:", input_line);
    fprintf(stderr, " error: ");
  } else
    fprintf(stderr, "makedep: error: ");
 ...
 */
static const char *strarray_get_value(const struct strarray *array,
                                      const char *name) {
 ...
 */
static void strarray_set_value(struct strarray *array, const char *name,
                               const char *value) {
  ...
  strarray_add(array, NULL);
  for (i = array->count - 1; i > min * 2 + 1; i--)
    array->str[i] = array->str[i - 2];
 ...
 *         replace_substr
 */
static char *replace_substr(const char *str, const char *start, size_t len,
                            const char *replace) {
...
static char *concat_paths(const char *base, const char *path) {
  ...
  if (!base || !base[0])
    return xstrdup(path && path[0] ? path : ".");
  if (!path || !path[0])
    return xstrdup(base);
  if (path[0] == '/')
    return xstrdup(path);
  ...
  while (len && !strncmp(path, "..", 2) && (!path[2] || path[2] == '/')) {
    for (i = len; i > 0; i--)
      if (base[i - 1] == '/')
        break;
    if (i == len - 2 && !memcmp(base + i, "..", 2))
      break; /* we can't go up if we already have ".." */
    if (i != len - 1 || base[i] != '.') {
      path += 2;
      while (*path == '/')
        path++;
 ...
 *
 * Check if the makefile was disabled for a PE arch that matches the native
 * arch.
 */
 ...
 *         get_link_arch
 */
static int get_link_arch(const struct makefile *make, unsigned int arch,
                         unsigned int *link_arch) {
  unsigned int hybrid_arch = hybrid_archs[arch];

  if (native_archs[arch])
    return 0;
  if (hybrid_arch && make->disabled[hybrid_arch])
    hybrid_arch = 0;
  if (make->disabled[arch] && !hybrid_arch)
    return 0;
 ...
 */
static int is_multiarch(unsigned int arch) { return archs.count == 1 || arch; }
...
static char *arch_make_variable(const char *name, unsigned int arch) {
  return arch ? strmake("$(%s_%s)", archs.str[arch], name)
              : strmake("$(%s)", name);
}
...
static char *tools_dir_path(const struct makefile *make, const char *path) {
  if (tools_dir)
    return strmake("%s/tools/%s", tools_dir, path);
 ...
 */
static void strarray_addall_path(struct strarray *array, const char *dir,
                                 struct strarray added) {
  unsigned int i;

  for (i = 0; i < added.count; i++)
    strarray_add(array, concat_paths(dir, added.str[i]));
}
...
static char *get_line(FILE *file) {
  ...
  for (;;) {
    ...
    while (p == buffer + size - 1 && p[-1] != '\n') {
      ...
      if (!fgets(buffer + size - 1, size + 1, file))
        break;
 ...
 *         add_dependency
 */
static void add_dependency(struct file *file, const char *name,
                           enum incl_type type) {
  if (file->deps_count >= file->deps_size) {
    file->deps_size *= 2;
    if (file->deps_size < 16)
      file->deps_size = 16;
 ...
 *         find_src_file
 */
static struct incl_file *find_src_file(const struct makefile *make,
                                       const char *name) {
  ...
  if (make == include_makefile) {
    ...
    LIST_FOR_EACH_ENTRY(file, &global_includes[hash], struct incl_file,
                        hash_entry)
 ...
 *         find_include_file
 */
static struct incl_file *find_include_file(const struct makefile *make,
                                           const char *name) {
  struct incl_file *file;
 ...
 */
static struct incl_file *add_include(struct makefile *make,
                                     struct incl_file *parent, const char *name,
                                     int line, enum incl_type type) {
  struct incl_file *include;

  if (parent->files_count >= parent->files_size) {
    parent->files_size *= 2;
    if (parent->files_size < 16)
      parent->files_size = 16;
    parent->files =
        xrealloc(parent->files, parent->files_size * sizeof(*parent->files));
  }

  LIST_FOR_EACH_ENTRY(include, &make->includes, struct incl_file, entry)
  if (!parent->use_msvcrt == !include->use_msvcrt &&
      !strcmp(name, include->name))
    goto found;
...
static void parse_pragma_directive(struct file *source, char *str) {
  ...
  while ((flag = strtok(NULL, " \t"))) {
    if (!strcmp(flag, "depend")) {
      while ((p = strtok(NULL, " \t")))
        add_dependency(source, p, INCL_NORMAL);
      return;
    } else if (!strcmp(flag, "install"))
      source->flags |= FLAG_INSTALL;
    else if (!strcmp(flag, "testdll"))
      source->flags |= FLAG_TESTDLL;

    if (strendswith(source->name, ".idl")) {
      if (!strcmp(flag, "header"))
        source->flags |= FLAG_IDL_HEADER;
      else if (!strcmp(flag, "proxy"))
        source->flags |= FLAG_IDL_PROXY;
      else if (!strcmp(flag, "client"))
        source->flags |= FLAG_IDL_CLIENT;
      else if (!strcmp(flag, "server"))
        source->flags |= FLAG_IDL_SERVER;
      else if (!strcmp(flag, "ident"))
        source->flags |= FLAG_IDL_IDENT;
      else if (!strcmp(flag, "typelib"))
        source->flags |= FLAG_IDL_TYPELIB;
      else if (!strcmp(flag, "register"))
        source->flags |= FLAG_IDL_REGISTER;
      else if (!strcmp(flag, "regtypelib"))
        source->flags |= FLAG_IDL_REGTYPELIB;
    } else if (strendswith(source->name, ".rc")) {
      if (!strcmp(flag, "header"))
        source->flags |= FLAG_RC_HEADER;
      else if (!strcmp(flag, "po"))
        source->flags |= FLAG_RC_PO;
      ...
      if (!strcmp(flag, "implib"))
        source->flags |= FLAG_C_IMPLIB;
      if (!strcmp(flag, "unix"))
        source->flags |= FLAG_C_UNIX;
      if (!strcmp(flag, "arm64ec_x64"))
        source->flags |= FLAG_ARM64EC_X64;
    }
  }
}
  ...
  while ((buffer = get_line(file))) {
    ...
    if (!strncmp(p, "cpp_quote", 9)) {
      ...
      if (quote == '"')
        p--; /* remove backslash */
      *p = 0;
      add_dependency(source, include,
                     (quote == '>') ? INCL_CPP_QUOTE_SYSTEM : INCL_CPP_QUOTE);
      continue;
    }
...
static void parse_in_file(struct file *source, FILE *file) {
  ...
  if (!strendswith(source->name, ".man.in"))
    return; /* not a man page */

  input_line = 0;
  while ((buffer = get_line(file))) {
    if (strncmp(buffer, ".TH", 3))
      continue;
      ...
      *eol = 0;
      ...
      if (*p++ == '#') {
        ...
        if (!strncmp(p, "pragma", 6))
          parse_pragma_directive(source, p + 6);
      }
      p = eol + 5;
    }
    p = skip_spaces(p);
...
static const struct {
  ...
  void (*parse)(struct file *file, FILE *f);
    ...
    {".c", parse_c_file},   {".h", parse_c_file},   {".inl", parse_c_file},
    {".l", parse_c_file},   {".m", parse_c_file},   {".rh", parse_c_file},
    {".x", parse_c_file},   {".y", parse_c_file},   {".idl", parse_idl_file},
    {".rc", parse_rc_file}, {".in", parse_in_file}, {".sfd", parse_sfd_file}};
...
static struct file *load_file(const char *name) {
  ...
  for (i = 0; i < ARRAY_SIZE(parse_functions); i++) {
    if (!strendswith(name, parse_functions[i].ext))
      continue;
 ...
 */
static struct file *open_include_path_file(const struct makefile *make,
 ...
 */
static struct file *open_file_same_dir(const struct incl_file *parent,
                                       const char *name, char **filename) {
  char *src_path = replace_filename(parent->file->name, name);
  struct file *ret = load_file(src_path);

  if (ret)
    *filename = replace_filename(parent->filename, name);
  return ret;
}
...
static struct file *open_same_dir_generated_file(const struct makefile *make,
  ...
  if (strendswith(file->name, ext) &&
      (ret = open_file_same_dir(
           parent, replace_extension(file->name, ext, src_ext), &filename))) {
    file->sourcename = filename;
    file->filename =
        obj_dir_path(make, replace_filename(parent->name, file->name));
  }
  return ret;
}
 ...
 */
static struct file *open_local_file(const struct makefile *make,
 ...
 */
static struct file *open_local_generated_file(const struct makefile *make,
                                              struct incl_file *file,
                                              const char *ext,
                                              const char *src_ext) {
  struct incl_file *include;
  ...
  if (strendswith(file->name, ext) &&
      (include =
           find_src_file(make, replace_extension(file->name, ext, src_ext)))) {
 ...
 */
static struct file *open_global_generated_file(const struct makefile *make,
                                               struct incl_file *file,
                                               const char *ext,
                                               const char *src_ext) {
  struct incl_file *include;
  ...
  if (strendswith(file->name, ext) &&
      (include = find_src_file(include_makefile,
                               replace_extension(file->name, ext, src_ext)))) {
    file->sourcename = include->filename;
 ...
 *         open_src_file
 */
static struct file *open_src_file(const struct makefile *make,
                                  struct incl_file *pFile) {
  struct file *file = open_local_file(make, pFile->name, &pFile->filename);

  if (!file)
    fatal_perror("open %s", pFile->name);
  return file;
}
...
static struct makefile *find_importlib_module(const char *name) {
  ...
  for (i = 0; i < subdirs.count; i++) {
    if (strncmp(submakes[i]->obj_dir, "dlls/", 5))
      continue;
    len = strlen(submakes[i]->obj_dir);
    if (strncmp(submakes[i]->obj_dir + 5, name, len - 5))
      continue;
    if (!name[len - 5] || !strcmp(name + len - 5, ".dll"))
      return submakes[i];
  }
  return NULL;
}
 ...
 *         open_include_file
 */
static struct file *open_include_file(const struct makefile *make,
                                      struct incl_file *pFile) {
  ...
  if ((file = open_local_generated_file(make, pFile, ".tab.h", ".y")))
    return file;
  if ((file = open_local_generated_file(make, pFile, ".h", ".idl")))
    return file;
  if (fontforge &&
      (file = open_local_generated_file(make, pFile, ".ttf", ".sfd")))
    return file;
  if (convert && rsvg && icotool) {
    if ((file = open_local_generated_file(make, pFile, ".bmp", ".svg")))
      return file;
    if ((file = open_local_generated_file(make, pFile, ".cur", ".svg")))
      return file;
    if ((file = open_local_generated_file(make, pFile, ".ico", ".svg")))
      return file;
  }
  if ((file = open_local_generated_file(make, pFile, "-client-protocol.h",
                                        ".xml")))
    return file;
  ...
  if ((file = open_local_file(make, pFile->name, &pFile->filename)))
    return file;
  ...
  if ((file = open_global_generated_file(make, pFile, ".h", ".idl")))
    return file;
  if ((file = open_global_generated_file(make, pFile, ".h", ".h.in")))
    return file;
  if (strendswith(pFile->name, "tmpl.h") &&
      (file = open_global_generated_file(make, pFile, ".h", ".x")))
    return file;

  /* check in global includes source dir */
  if ((file = open_global_header(pFile->name, &pFile->filename)))
    return file;

  /* check in global msvcrt includes */
  if (pFile->use_msvcrt &&
      (file = open_global_header(strmake("msvcrt/%s", pFile->name),
                                 &pFile->filename)))
    return file;
  ...
  for (i = 0; i < make->include_paths.count; i++) {
    ...
    if (root_src_dir) {
      ...
      if (!strncmp(dir, root_src_dir, len) && (!dir[len] || dir[len] == '/')) {
        while (dir[len] == '/')
          len++;
        file = open_global_file(concat_paths(dir + len, pFile->name),
                                &pFile->filename);
      }
    } else {
      if (*dir == '/')
  ...
  if (pFile->type == INCL_SYSTEM)
    return NULL; /* ignore system files we cannot find */

  /* try in src file directory */
  if ((file = open_same_dir_generated_file(make, pFile->included_by, pFile,
                                           ".tab.h", ".y")) ||
      (file = open_same_dir_generated_file(make, pFile->included_by, pFile,
                                           ".h", ".idl")) ||
      (file = open_file_same_dir(pFile->included_by, pFile->name,
                                 &pFile->filename))) {
  ...
  if (make->extlib)
    return NULL; /* ignore missing files in external libs */

  fprintf(stderr, "%s:%d: error: ", pFile->included_by->file->name,
          pFile->included_line);
  perror(pFile->name);
  ...
  while (pFile && pFile->included_by) {
    ...
    if (!parent)
      parent = pFile->included_by->file->name;
    fprintf(stderr, "%s:%d: note: %s was first included here\n", parent,
            pFile->included_line, pFile->name);
    pFile = pFile->included_by;
  }
  exit(1);
}
 ...
 *         add_all_includes
 */
static void add_all_includes(struct makefile *make, struct incl_file *parent,
                             struct file *file) {
  unsigned int i;
  ...
  for (i = 0; i < file->deps_count; i++) {
    ...
    case INCL_NORMAL:
    case INCL_IMPORT:
      add_include(make, parent, file->deps[i].name, file->deps[i].line,
                  INCL_NORMAL);
      break;
    case INCL_IMPORTLIB:
      add_include(make, parent, file->deps[i].name, file->deps[i].line,
 ...
 *         parse_file
 */
static void parse_file(struct makefile *make, struct incl_file *source,
                       int src) {
  struct file *file =
      src ? open_src_file(make, source) : open_include_file(make, source);
  ...
  if (strendswith(file->name, ".m"))
    file->flags |= FLAG_C_UNIX;
  if (file->flags & FLAG_C_UNIX)
    source->use_msvcrt = 0;
  else if (file->flags & FLAG_C_IMPLIB)
    source->use_msvcrt = 1;
      ...
      for (i = 0; i < file->deps_count; i++) {
        ...
        case INCL_IMPORT:
          if (strendswith(file->deps[i].name, ".idl"))
            add_include(make, source,
                        replace_extension(file->deps[i].name, ".idl", ".h"),
                        file->deps[i].line, INCL_NORMAL);
          else
            add_include(make, source, file->deps[i].name, file->deps[i].line,
                        INCL_NORMAL);
          break;
        case INCL_CPP_QUOTE:
          add_include(make, source, file->deps[i].name, file->deps[i].line,
...
static FILE *open_input_makefile(const struct makefile *make) {
  ...
  if (make->obj_dir)
    ...
    input_file_name =
        output_makefile_name; /* always use output name for main Makefile */
 ...
 *         get_make_variable
 */
static const char *get_make_variable(const struct makefile *make,
                                     const char *name) {
  const char *ret;

  if ((ret = strarray_get_value(&cmdline_vars, name)))
    return ret;
  if ((ret = strarray_get_value(&make->vars, name)))
    return ret;
 ...
 */
static char *get_expanded_make_variable(const struct makefile *make,
                                        const char *name) {
  const char *var;
  ...
  while ((p = strchr(p, '$'))) {
    if (p[1] == '(') {
      if (!(end = strchr(p + 2, ')')))
        fatal_error("syntax error in '%s'\n", expand);
      *end++ = 0;
      if (strchr(p + 2, ':'))
        fatal_error("pattern replacement not supported for '%s'\n", p + 2);
 ...
 */
static struct strarray get_expanded_make_var_array(const struct makefile *make,
                                                   const char *name) {
 ...
 */
static struct strarray get_expanded_file_local_var(const struct makefile *make,
                                                   const char *file,
                                                   const char *name) {
 ...
 */
static char *get_expanded_arch_var(const struct makefile *make,
                                   const char *name, int arch) {
  return get_expanded_make_variable(
      make, arch ? strmake("%s_%s", archs.str[arch], name) : name);
}

/*******************************************************************
 *         get_expanded_arch_var_array
 */
static struct strarray get_expanded_arch_var_array(const struct makefile *make,
                                                   const char *name, int arch) {
  return get_expanded_make_var_array(
      make, arch ? strmake("%s_%s", archs.str[arch], name) : name);
}
...
static struct makefile *parse_makefile(const char *path) {
  ...
  make->obj_dir = path;
  if (root_src_dir)
    make->src_dir = root_src_dir_path(make->obj_dir);
  if (path && !strcmp(path, "include"))
    include_makefile = make;
...
static void add_generated_sources(struct makefile *make) {
  ...
  LIST_FOR_EACH_ENTRY_SAFE(source, next, &make->sources, struct incl_file,
                           entry) {
    for (arch = 0; arch < archs.count; arch++) {
      if (!is_multiarch(arch))
        continue;
      if (source->file->flags & FLAG_IDL_CLIENT) {
        file = add_generated_source(
            make, replace_extension(source->name, ".idl", "_c.c"), NULL, arch);
        add_dependency(file->file,
                       replace_extension(source->name, ".idl", ".h"),
                       INCL_NORMAL);
        add_all_includes(make, file, file->file);
      }
      if (source->file->flags & FLAG_IDL_SERVER) {
        file = add_generated_source(
            make, replace_extension(source->name, ".idl", "_s.c"), NULL, arch);
      ...
      if (source->file->flags & FLAG_IDL_IDENT) {
        file = add_generated_source(
            make, replace_extension(source->name, ".idl", "_i.c"), NULL, arch);
      ...
      if (source->file->flags & FLAG_IDL_PROXY) {
        file = add_generated_source(
            make, replace_extension(source->name, ".idl", "_p.c"), NULL, arch);
      ...
      if (source->file->flags & FLAG_IDL_TYPELIB) {
        add_generated_source(make,
                             replace_extension(source->name, ".idl", "_l.res"),
                             NULL, arch);
      }
      if (source->file->flags & FLAG_IDL_REGTYPELIB) {
        add_generated_source(make,
                             replace_extension(source->name, ".idl", "_t.res"),
                             NULL, arch);
      }
      if (source->file->flags & FLAG_IDL_REGISTER) {
        add_generated_source(make,
                             replace_extension(source->name, ".idl", "_r.res"),
                             NULL, arch);
      }
    }
    ...
    if (source->file->flags & FLAG_IDL_HEADER) {
      add_generated_source(make, replace_extension(source->name, ".idl", ".h"),
                           NULL, 0);
    }
    if (!source->file->flags && strendswith(source->name, ".idl")) {
      if (!strncmp(source->name, "wine/", 5))
        continue;
    ...
    if (strendswith(source->name, ".x")) {
      add_generated_source(make, replace_extension(source->name, ".x", ".h"),
                           NULL, 0);
    }
    if (strendswith(source->name, ".y")) {
      file = add_generated_source(
          make, replace_extension(source->name, ".y", ".tab.c"), NULL, 0);
    ...
    if (strendswith(source->name, ".l")) {
      file = add_generated_source(
          make, replace_extension(source->name, ".l", ".yy.c"), NULL, 0);
      /* steal the includes list from the source file */
      file->files_count = source->files_count;
      file->files_size = source->files_size;
      file->files = source->files;
    ...
    if (strendswith(source->name, ".xml")) {
      ...
      char *header_name =
          replace_extension(source->name, ".xml", "-client-protocol.h");
  ...
  for (i = 0; i < objs.count; i++) {
    ...
    if (strendswith(objs.str[i], ".o")) {
      file = add_generated_source(
          make, objs.str[i], replace_extension(objs.str[i], ".o", ".c"), 0);
      file->file->flags |= FLAG_C_UNIX;
      file->use_msvcrt = 0;
    } else if (strendswith(objs.str[i], ".res"))
      add_generated_source(make, replace_extension(objs.str[i], ".res", ".rc"),
                           NULL, 0);
    else
 ...
 */
static void create_file_directories(const struct makefile *make,
  ...
  for (i = 0; i < subdirs.count; i++)
    create_dir(subdirs.str[i]);
}

/*******************************************************************
 *         output_filenames_obj_dir
 */
static void output_filenames_obj_dir(const struct makefile *make,
                                     struct strarray array) {
  unsigned int i;

  for (i = 0; i < array.count; i++)
    output_filename(obj_dir_path(make, array.str[i]));
}
...
static void get_dependencies(struct incl_file *file, struct incl_file *source) {
  ...
  for (i = 0; i < file->files_count; i++)
    get_dependencies(file->files[i], source);
}
 ...
 */
static struct strarray get_local_dependencies(const struct makefile *make,
                                              const char *name,
 ...
 *         get_static_lib
 *
 * Find the makefile that builds the named static library (which may be an
 * import lib).
 */
static struct makefile *get_static_lib(const char *name, unsigned int arch) {
  ...
  for (i = 0; i < subdirs.count; i++) {
    if (submakes[i]->importlib && !strcmp(submakes[i]->importlib, name))
      return submakes[i];
    if (!submakes[i]->staticlib)
      continue;
    if (submakes[i]->disabled[arch])
      continue;
    if (strncmp(submakes[i]->staticlib, "lib", 3))
      continue;
    if (strncmp(submakes[i]->staticlib + 3, name, strlen(name)))
      continue;
    if (strcmp(submakes[i]->staticlib + 3 + strlen(name), ".a"))
      continue;
    return submakes[i];
  }
  return NULL;
 ...
 *         get_native_unix_lib
 */
static const char *get_native_unix_lib(const struct makefile *make,
                                       const char *name) {
  if (!make->unixlib)
    return NULL;
  if (strncmp(make->unixlib, name, strlen(name)))
    return NULL;
  if (make->unixlib[strlen(name)] != '.')
    return NULL;
...
static struct makefile *get_parent_makefile(struct makefile *make) {
  ...
  *p = 0;
  for (i = 0; i < subdirs.count; i++)
    if (!strcmp(submakes[i]->obj_dir, dir))
      return submakes[i];
  return NULL;
}
 ...
 *         add_unix_libraries
 */
static struct strarray add_unix_libraries(const struct makefile *make,
                                          struct strarray *deps) {
  struct strarray ret = empty_strarray;
  struct strarray all_libs = empty_strarray;
  unsigned int i, j;

  if (strcmp(make->unixlib, "ntdll.so"))
    strarray_add(&all_libs, "-lntdll");
  ...
  for (i = 0; i < all_libs.count; i++) {
    ...
    if (!strncmp(all_libs.str[i], "-l", 2)) {
      for (j = 0; j < subdirs.count; j++) {
        if (make == submakes[j])
          continue;
        if ((lib = get_native_unix_lib(submakes[j], all_libs.str[i] + 2)))
          break;
      }
    }
    if (lib) {
      ...
      strarray_add(&ret, all_libs.str[i]);
  }
...
static int is_crt_module(const char *file) {
  return !strncmp(file, "msvcr", 5) || !strncmp(file, "ucrt", 4) ||
         !strcmp(file, "crtdll.dll");
}
...
static const char *get_default_crt(const struct makefile *make) {
  if (make->module && is_crt_module(make->module))
    return NULL; /* don't add crt import to crt dlls */
  return !make->testdll && (!make->staticlib || make->extlib) ? "ucrtbase"
                                                              : "msvcrt";
}
...
static const char *get_crt_define(const struct makefile *make) {
  ...
  for (i = 0; i < make->imports.count; i++) {
    if (!is_crt_module(make->imports.str[i]))
      continue;
    if (crt_dll)
  ...
  if (!crt_dll) {
    ...
    if (!(crt_dll = get_default_crt(make)))
      crt_dll = make->module;
  }
  if (!strncmp(crt_dll, "ucrt", 4))
    return "-D_UCRT";
 ...
 *         add_import_libs
 */
static struct strarray add_import_libs(const struct makefile *make,
                                       struct strarray *deps,
                                       struct strarray imports,
  ...
  if (!get_link_arch(make, arch, &link_arch))
    return ret;
  ...
  for (i = 0; i < imports.count; i++) {
    ...
    if (is_crt_module(imports.str[i]) && type != IMPORT_TYPE_DEFAULT)
      continue;
    ...
    if ((submake = get_static_lib(name, link_arch))) {
      const char *ext = (type == IMPORT_TYPE_DELAYED && !delay_load_flags[arch])
                            ? ".delay.a"
                            : ".a";
      const char *lib = obj_dir_path(
          submake, strmake("%slib%s%s", arch_dirs[arch], name, ext));
...
static void add_install_rule(struct makefile *make, const char *target,
                             unsigned int arch, const char *file,
                             const char *dest) {
  unsigned int i;

  if (make->disabled[arch])
    return;
...
static const char *get_include_install_path(const char *name) {
  if (!strncmp(name, "wine/", 5))
    return name + 5;
  if (!strncmp(name, "msvcrt/", 7))
    return name;
 ...
 *         get_source_defines
 */
static struct strarray get_source_defines(struct makefile *make,
...
static const char *get_debug_file(struct makefile *make, const char *name,
  ...
  if (!debug_flags[arch])
    return NULL;
  if (!strcmp(debug_flags[arch], "pdb"))
    debug_file = strmake("%s.pdb", get_base_name(name));
  else if (!strncmp(debug_flags[arch], "split", 5))
    debug_file = strmake("%s.debug", name);
  if (debug_file)
 ...
 *
 * Output rule to create a symlink back to the source directory, for source
 * files that are needed at run-time.
 */
 ...
 */
static void output_install_commands(struct makefile *make,
                                    struct strarray files) {
  unsigned int i, arch;
  ...
  for (i = 0; i < files.count; i += 2) {
    ...
    case 'd': /* data file */
      output("\t%s -m 644 $(INSTALL_DATA_FLAGS) %s %s\n", install_sh,
             obj_dir_path(make, file), dest);
      break;
    case 'D': /* data file in source dir */
      output("\t%s -m 644 $(INSTALL_DATA_FLAGS) %s %s\n", install_sh,
             src_dir_path(make, file), dest);
      break;
    ...
    case 't': /* script in tools dir */
      output("\t%s $(INSTALL_SCRIPT_FLAGS) %s %s\n", install_sh,
             tools_dir_path(make, files.str[i]), dest);
      break;
    case 'y': /* symlink */
...
static void output_uninstall_rules(struct makefile *make) {
  static const char *dirs_order[] = {"$(includedir)", "$(mandir)",
                                     "$(datadir)"};

  struct strarray uninstall_dirs;
  unsigned int i, j;

  if (!make->uninstall_files.count)
    return;
  output("uninstall::\n");
  ...
  for (i = 0; i < ARRAY_SIZE(dirs_order); i++) {
    for (j = 0; j < uninstall_dirs.count; j++) {
      if (!uninstall_dirs.str[j])
        continue;
      if (strncmp(uninstall_dirs.str[j] + strlen("$(DESTDIR)"), dirs_order[i],
                  strlen(dirs_order[i])))
        continue;
...
static void output_po_files(struct makefile *make) {
  ...
  if (linguas.count) {
    ...
    output(": %s/wine.pot\n", po_dir);
    output("\t%smsgmerge --previous -q $@ %s/wine.pot | msgattrib "
           "--no-obsolete -o $@.new && mv $@.new $@\n",
 ...
 *         output_source_y
 */
static void output_source_y(struct makefile *make, struct incl_file *source,
                            const char *obj) {
 ...
 *         output_source_l
 */
static void output_source_l(struct makefile *make, struct incl_file *source,
                            const char *obj) {
  output("%s.yy.c: %s\n", obj_dir_path(make, obj), source->filename);
 ...
 *         output_source_h
 */
static void output_source_h(struct makefile *make, struct incl_file *source,
                            const char *obj) {
  if (source->file->flags & FLAG_GENERATED)
    strarray_add(&make->all_targets[0], source->name);
  else if ((source->file->flags & FLAG_INSTALL) ||
           strncmp(source->name, "wine/", 5))
    add_install_rule(make, source->name, 0, source->name,
                     strmake("D$(includedir)/wine/%s",
                             get_include_install_path(source->name)));
}

/*******************************************************************
 *         output_source_rc
 */
static void output_source_rc(struct makefile *make, struct incl_file *source,
                             const char *obj) {
  struct strarray defines = get_source_defines(make, source, obj);
  const char *po_dir = NULL, *res_file = strmake("%s.res", obj);
  unsigned int i, arch;

  if (source->file->flags & FLAG_RC_HEADER)
    return;
  if (source->file->flags & FLAG_GENERATED)
    strarray_add(&make->clean_files, source->name);
  if (linguas.count && (source->file->flags & FLAG_RC_PO))
    po_dir = "po";
  if (!(source->file->flags & FLAG_TESTDLL)) {
    for (arch = 0; arch < archs.count; arch++)
      if (!make->disabled[arch])
        strarray_add(&make->res_files[arch], res_file);
  } else
    strarray_add(&make->clean_files, res_file);
 ...
 *         output_source_mc
 */
static void output_source_mc(struct makefile *make, struct incl_file *source,
                             const char *obj) {
  unsigned int i, arch;
  ...
  if (make->src_dir)
    output_filename("nls/locale.nls");
  output("\n");
  output("\t%s%s -u -o $@ %s", cmd_prefix("WMC"), tools_path(make, "wmc"),
         source->filename);
  if (linguas.count) {
 ...
 *         output_source_res
 */
static void output_source_res(struct makefile *make, struct incl_file *source,
                              const char *obj) {
  if (make->disabled[source->arch])
    return;
 ...
 *         output_source_idl
 */
static void output_source_idl(struct makefile *make, struct incl_file *source,
                              const char *obj) {
  struct strarray defines = get_source_defines(make, source, obj);
  ...
  for (arch = 0; arch < archs.count; arch++) {
    ...
    if (!arch)
      strarray_addall(&arch_deps, headers);
    ...
    if (arch)
      output_filenames(
          get_expanded_arch_var_array(make, "EXTRAIDLFLAGS", arch));
    output_filename(source->filename);
    output("\n");
  ...
  if (source->importlibdeps.count) {
    for (arch = 0; arch < archs.count; arch++) {
      if (!multiarch_targets[arch].count)
        continue;
      ...
      for (i = 0; i < source->importlibdeps.count; i++) {
        ...
        struct makefile *submake =
            find_importlib_module(source->importlibdeps.str[i]);
        const char *module =
            strmake("%s%s", arch_pe_dirs[native_arch], submake->module);
 ...
 *         output_source_x
 */
static void output_source_x(struct makefile *make, struct incl_file *source,
                            const char *obj) {
...
static void output_source_sfd(struct makefile *make, struct incl_file *source,
  ...
  if (fontforge && !make->src_dir) {
    ...
    output("\t%s%s -script %s %s $@\n", cmd_prefix("GEN"), fontforge,
           root_src_dir_path("fonts/genttf.ff"), source->filename);
    if (!(source->file->flags & FLAG_SFD_FONTS))
      strarray_add(&make->font_files, ttf_obj);
    strarray_add(&make->maintainerclean_files, ttf_obj);
  }
  if (source->file->flags & FLAG_INSTALL) {
    add_install_rule(make, source->name, 0, ttf_obj,
                     strmake("D$(datadir)/wine/fonts/%s", ttf_obj));
  ...
  if (source->file->flags & FLAG_SFD_FONTS) {
    ...
    for (i = 0; i < array->count; i++) {
      ...
      add_install_rule(make, source->name, 0, xstrdup(font),
                       strmake("d$(datadir)/wine/fonts/%s", font));
    }
  }
}
 ...
 *         output_source_svg
 */
static void output_source_svg(struct makefile *make, struct incl_file *source,
                              const char *obj) {
  ...
  if (convert && rsvg && icotool) {
    for (i = 0; images[i]; i++)
      if (find_include_file(make, strmake("%s.%s", obj, images[i])))
        break;

    if (images[i]) {
      output("%s.%s: %s\n", src_dir_path(make, obj), images[i],
             source->filename);
      ...
      strarray_add(&make->maintainerclean_files,
                   strmake("%s.%s", obj, images[i]));
    }
  }
}
 ...
 *         output_source_nls
 */
static void output_source_nls(struct makefile *make, struct incl_file *source,
                              const char *obj) {
  add_install_rule(make, source->name, 0, source->name,
 ...
 */
static void output_source_desktop(struct makefile *make,
                                  struct incl_file *source, const char *obj) {
  add_install_rule(make, source->name, 0, source->name,
 ...
 *         output_source_po
 */
static void output_source_po(struct makefile *make, struct incl_file *source,
                             const char *obj) {
  output("%s.mo: %s\n", obj_dir_path(make, obj), source->filename);
 ...
 *         output_source_in
 */
static void output_source_in(struct makefile *make, struct incl_file *source,
                             const char *obj) {
  unsigned int i;
 ...
 *         output_source_spec
 */
static void output_source_spec(struct makefile *make, struct incl_file *source,
                               const char *obj) {
  /* nothing to do */
}
 ...
 */
static void output_source_testdll(struct makefile *make,
                                  struct incl_file *source, const char *obj) {
  ...
  const char *dll_name, *obj_name, *res_name, *output_rsrc, *output_file,
      *debug_file, *ext = ".dll";
  ...
  strarray_addall(&dll_flags,
                  get_expanded_file_local_var(make, obj, "EXTRADLLFLAGS"));
  if (!strarray_exists(&dll_flags, "-nodefaultlibs"))
    default_imports = get_default_imports(make, imports);
  if (strarray_exists(&dll_flags, "-mconsole"))
    ext = ".exe";

  for (arch = 0; arch < archs.count; arch++) {
    const char *hybrid_obj_name = NULL;

    if (!is_multiarch(arch) || !get_link_arch(make, arch, &link_arch))
      continue;

    all_libs = dep_libs = empty_strarray;
    strarray_addall(&all_libs, add_import_libs(make, &dep_libs, imports,
                                               IMPORT_TYPE_DIRECT, arch));
    strarray_addall(&all_libs, add_import_libs(make, &dep_libs, default_imports,
                                               IMPORT_TYPE_DEFAULT, arch));
    if (!arch)
    ...
    if (link_arch != arch)
      hybrid_obj_name =
          obj_dir_path(make, strmake("%s%s.o", arch_dirs[link_arch], obj));
    output_file = obj_dir_path(make, dll_name);
    output_rsrc = strmake("%s.res", dll_name);

    if (!find_src_file(make, strmake("%s.rc", obj)))
      res_name = NULL;
    else
      res_name = obj_dir_path(make, strmake("%s.res", obj));
    ...
    if (hybrid_obj_name)
      output_filename(hybrid_obj_name);
    if (res_name)
    ...
    if (link_arch)
      output_filenames(
          get_expanded_arch_var_array(make, "EXTRADLLFLAGS", link_arch));
    if (!strcmp(ext, ".dll"))
    ...
    if ((debug_file = get_debug_file(make, dll_name, link_arch)))
      output_filename(
          strmake("-Wl,--debug-file,%s", obj_dir_path(make, debug_file)));
 ...
 *         output_source_xml
 */
static void output_source_xml(struct makefile *make, struct incl_file *source,
                              const char *obj) {
  if (wayland_scanner) {
    output("%s-protocol.c: %s\n", obj_dir_path(make, obj), source->filename);
    output("\t%s%s private-code %s $@\n", cmd_prefix("GEN"), wayland_scanner,
           source->filename);
 ...
 */
static void output_source_one_arch(struct makefile *make,
                                   struct incl_file *source, const char *obj,
                                   struct strarray defines,
                                   struct strarray *targets,
                                   unsigned int arch) {
  ...
  if (arch) {
    if (source->file->flags & FLAG_C_UNIX)
      return;
    if (!is_using_msvcrt(make) && !make->staticlib &&
  ...
  if (!source->use_msvcrt)
    strarray_addall(&cflags, make->unix_cflags);
  if ((source->file->flags & FLAG_ARM64EC_X64) &&
      !strcmp(archs.str[arch], "arm64ec")) {
    var_cc = "$(x86_64_CC)";
    var_cflags = "$(x86_64_CFLAGS)";
    strarray_add(&cflags, "-D__arm64ec_x64__");
    strarray_addall(&cflags, get_expanded_make_var_array(top_makefile,
                                                         "x86_64_EXTRACFLAGS"));
  } else {
    var_cc = arch_make_variable("CC", arch);
    var_cflags = arch_make_variable("CFLAGS", arch);
    strarray_addall(&cflags, make->extlib ? extra_cflags_extlib[arch]
                                          : extra_cflags[arch]);
  }
  ...
  if (!arch) {
    if (source->file->flags & FLAG_C_UNIX) {
      ...
      if (source->use_msvcrt)
        strarray_addall(&cflags, msvcrt_flags);
  ...
  if (make->testdll && strendswith(source->name, ".c") &&
    ...
    output("\t%s%s $(RUNTESTFLAGS) -T . -M %s -p %s %s && touch $@\n",
           cmd_prefix("TEST"), root_src_dir_path("tools/runtest"),
           make->testdll, obj_dir_path(make, arch_module_name(test_exe, arch)),
           obj);
  }
  ...
  if (sarif_converter && make->module && !make->extlib) {
    const char *sast_name =
        strmake("%s%s.sarif", source->arch ? "" : arch_dirs[arch], obj);
 ...
 */
static void output_source_default(struct makefile *make,
                                  struct incl_file *source, const char *obj) {
  struct strarray defines = get_source_defines(make, source, obj);
  struct strarray targets = empty_strarray;
  unsigned int arch;
...
} output_source_funcs[] = {{"y", output_source_y},
                           {"l", output_source_l},
                           {"h", output_source_h},
                           {"rh", output_source_h},
                           {"inl", output_source_h},
...
static void output_fake_module(struct makefile *make, const char *spec_file) {
  ...
  add_install_rule(make, make->module, arch, name,
                   strmake("d$(libdir)/wine/%s", name));
...
static void output_module(struct makefile *make, unsigned int arch) {
  ...
  if (!make->is_exe) {
    if (make->data_only ||
      ...
      struct incl_file *spec =
          find_src_file(make, replace_extension(make->module, ".dll", ".spec"));
      if (spec)
        spec_file = spec->filename;
    } else
      spec_file =
          src_dir_path(make, replace_extension(make->module, ".dll", ".spec"));
  }
  ...
  if (!make->data_only) {
    ...
    strarray_addall(&all_libs,
                    add_import_libs(make, &dep_libs, make->delayimports,
                                    IMPORT_TYPE_DELAYED, arch));
    strarray_addall(&all_libs, add_import_libs(make, &dep_libs, default_imports,
                                               IMPORT_TYPE_DEFAULT, arch));
    if (!arch)
      strarray_addall(&all_libs, libs);
    ...
    if (delay_load_flags[arch]) {
      for (i = 0; i < make->delayimports.count; i++) {
        struct makefile *import =
            get_static_lib(make->delayimports.str[i], arch);
        if (import)
          strarray_add(&all_libs,
                       strmake("%s%s", delay_load_flags[arch], import->module));
      }
    }
  } else {
    if (native_archs[arch])
  ...
  if (link_arch != arch)
    output_filenames_obj_dir(make, make->object_files[link_arch]);
  output_filenames_obj_dir(make, make->res_files[arch]);
  output_filenames(dep_libs);
  output_filename(tools_path(make, "winebuild"));
  output_filename(tools_path(make, "winegcc"));
  ...
  if (!make->data_only && !arch && unix_lib_supported)
...
static void output_import_lib(struct makefile *make, unsigned int arch) {
  ...
  output("%s: %s %s", obj_dir_path(make, name), tools_path(make, "winebuild"),
         spec_file);
  output_filenames_obj_dir(make, make->implib_files[arch]);
  if (hybrid_arch)
    output_filenames_obj_dir(make, make->implib_files[hybrid_arch]);
  output("\n");
  output("\t%s%s -w --implib -o $@", cmd_prefix("BUILD"),
         tools_path(make, "winebuild"));
  if (!delay_load_flags[arch])
  ...
  if (make->is_win16)
    output_filename("-m16");
  if (hybrid_arch)
    output_filenames(hybrid_target_flags[hybrid_arch]);
...
static void output_static_lib(struct makefile *make, unsigned int arch) {
  ...
  output_filenames_obj_dir(make, make->object_files[arch]);
  if (hybrid_arch)
    output_filenames_obj_dir(make, make->object_files[hybrid_arch]);
  if (!arch)
    output_filenames_obj_dir(make, make->unixobj_files);
  output("\n");
  output("\t%s%s -w --staticlib -o $@", cmd_prefix("BUILD"),
         tools_path(make, "winebuild"));
  output_filenames(target_flags[hybrid_arch ? hybrid_arch : arch]);
  output_filenames_obj_dir(make, make->object_files[arch]);
  if (hybrid_arch)
...
static void output_test_module(struct makefile *make, unsigned int arch) {
  ...
  char *stripped =
      arch_module_name(strmake("%s_test-stripped.exe", basemodule), arch);
  ...
  strarray_addall(&all_libs, add_import_libs(make, &dep_libs, make->imports,
                                             IMPORT_TYPE_DIRECT, arch));
  strarray_addall(&all_libs, add_import_libs(make, &dep_libs, default_imports,
                                             IMPORT_TYPE_DEFAULT, arch));
  ...
  output_filename(arch_make_variable("LDFLAGS", link_arch));
  output("\n");
  output("%s %s:", obj_dir_path(make, testmodule),
         obj_dir_path(make, stripped));
  output_filenames_obj_dir(make, make->object_files[arch]);
  if (link_arch != arch)
    output_filenames_obj_dir(make, make->object_files[link_arch]);
  ...
  output("\t%secho \"%s_test.exe TESTRES \\\"%s\\\"\" | %s -u -o $@\n",
         cmd_prefix("WRC"), basemodule, obj_dir_path(make, stripped),
         tools_path(make, "wrc"));
  ...
  if (parent) {
    ...
    if (parent->unixlib)
      output_filename(obj_dir_path(parent, parent->unixlib));
  }
  output("\n");
...
static void output_programs(struct makefile *make) {
  ...
  for (i = 0; i < make->programs.count; i++) {
    ...
    struct strarray deps =
        get_local_dependencies(make, make->programs.str[i], make->in_files);
    struct strarray all_libs =
        get_expanded_file_local_var(make, make->programs.str[i], "LDFLAGS");
    struct strarray objs =
        get_expanded_file_local_var(make, make->programs.str[i], "OBJS");
    struct strarray symlinks =
        get_expanded_file_local_var(make, make->programs.str[i], "SYMLINKS");

    if (!objs.count)
      objs = make->object_files[arch];
    if (!strarray_exists(&all_libs, "-nodefaultlibs")) {
      strarray_addall(&all_libs,
                      get_expanded_make_var_array(make, "UNIX_LIBS"));
      strarray_addall(&all_libs, libs);
    }
    ...
    for (j = 0; j < symlinks.count; j++) {
      output("%s: %s\n", obj_dir_path(make, symlinks.str[j]),
    ...
    add_install_rule(make, program, arch,
                     program_installed ? program_installed : program,
...
static void output_subdirs(struct makefile *make) {
  ...
  for (arch = 0; arch < archs.count; arch++)
    strarray_addall(&all_targets, make->all_targets[arch]);
  for (i = 0; i < subdirs.count; i++) {
    struct strarray subclean = empty_strarray;
    strarray_addall(&subclean, get_removable_dirs(submakes[i]->clean_files));
    strarray_addall(&subclean,
                    get_removable_dirs(submakes[i]->distclean_files));
    ...
    strarray_addall_path(&clean_files, submakes[i]->obj_dir,
                         submakes[i]->clean_files);
    strarray_addall_path(&sast_files, submakes[i]->obj_dir,
                         submakes[i]->sast_files);
    strarray_addall_path(&distclean_files, submakes[i]->obj_dir,
                         submakes[i]->distclean_files);
    strarray_addall_path(&distclean_dirs, submakes[i]->obj_dir, subclean);
    strarray_addall_path(&make->maintainerclean_files, submakes[i]->obj_dir,
                         submakes[i]->maintainerclean_files);
    strarray_addall_path(&make->pot_files, submakes[i]->obj_dir,
                         submakes[i]->pot_files);

    for (arch = 0; arch < archs.count; arch++) {
      if (submakes[i]->disabled[arch])
        continue;
      strarray_addall_path(&all_targets, submakes[i]->obj_dir,
                           submakes[i]->all_targets[arch]);
      strarray_addall_path(&testclean_files, submakes[i]->obj_dir,
                           submakes[i]->ok_files[arch]);
    }
    if (submakes[i]->disabled[0])
      continue;

    strarray_addall_path(&all_targets, submakes[i]->obj_dir,
                         submakes[i]->font_files);
    if (!strcmp(submakes[i]->obj_dir, "tools") ||
        !strncmp(submakes[i]->obj_dir, "tools/", 6))
    ...
    for (j = 0; j < NB_INSTALL_RULES; j++)
      if (submakes[i]->install_rules[j].count)
        strarray_add(&install_deps[j],
                     obj_dir_path(submakes[i], install_targets[j]));
  }
  ...
  if (get_expanded_make_variable(make, "GETTEXTPO_LIBS"))
    output_po_files(make);
...
static void output_sources(struct makefile *make) {
  ...
  LIST_FOR_EACH_ENTRY(source, &make->sources, struct incl_file, entry) {
    ...
    *ext++ = 0;
    ...
    for (j = 0; output_source_funcs[j].ext; j++)
      if (!strcmp(ext, output_source_funcs[j].ext))
        break;
  ...
  if (make->obj_dir && !strcmp(make->obj_dir, "programs/winetest")) {
    for (arch = 0; arch < archs.count; arch++) {
      ...
      for (i = 0; i < subdirs.count; i++) {
        ...
        if (submakes[i]->disabled[arch])
          continue;
        if (enable_tests.count &&
            !strarray_exists(&enable_tests, submakes[i]->testdll))
          continue;
        strarray_add(&make->res_files[arch],
                     strmake("%s%s", arch_dirs[arch],
                             replace_extension(submakes[i]->testdll, ".dll",
                                               "_test.res")));
      }
    }
  }
  ...
  if (make->dlldata_files.count) {
    ...
    output("\t%s%s --dlldata-only -o $@", cmd_prefix("WIDL"),
  ...
  if (make->staticlib) {
    ...
    for (arch = 0; arch < archs.count; arch++) {
      ...
      if (make->importlib &&
          (is_multiarch(arch) || !is_native_arch_disabled(make)))
        output_import_lib(make, arch);
    }
    if (make->unixlib)
      output_unix_lib(make);
    if (make->is_exe && !make->is_win16 && unix_lib_supported &&
        strendswith(make->module, ".exe")) {
      char *binary = replace_extension(make->module, ".exe", "");
      add_install_rule(make, binary, 0, "wineapploader",
                       strmake("t$(bindir)/%s", binary));
    }
  } else if (make->testdll) {
    for (arch = 0; arch < archs.count; arch++)
      if (is_multiarch(arch))
        output_test_module(make, arch);
  } else if (make->programs.count)
  ...
  for (i = 0; i < make->extra_targets.count; i++)
    if (strarray_exists(&make->dependencies,
                        obj_dir_path(make, make->extra_targets.str[i])))
      strarray_add(&make->clean_files, make->extra_targets.str[i]);
    else
      strarray_add(&make->all_targets[0], make->extra_targets.str[i]);

  if (!make->src_dir)
    strarray_add(&make->distclean_files, ".gitignore");
  strarray_add(&make->distclean_files, "Makefile");
  if (make->testdll)
    strarray_add(&make->distclean_files, "testlist.c");

  if (!make->obj_dir)
    strarray_addall(&make->distclean_files,
                    get_expanded_make_var_array(make, "CONFIGURE_TARGETS"));
...
static void output_compile_commands(const char *dest) {
  ...
  LIST_FOR_EACH_ENTRY(cmd, &compile_commands, struct compile_command, entry) {
    output("  {\n");
    output("    \"command\": \"%s -c -o %s %s", cmd->cmd, cmd->obj,
           cmd->source);
    for (i = 0; i < cmd->args.count; i++)
      output(" %s", escape_cstring(cmd->args.str[i]));
    output("\",\n");
    output("    \"file\": \"%s\",\n", cmd->source);
    output("    \"output\": \"%s\",\n", cmd->obj);
    output("    \"directory\": \"%s\"\n", dir);
...
static void output_stub_makefile(struct makefile *make) {
  ...
  for (arch = 0; arch < archs.count; arch++)
    if (make->all_targets[arch].count)
      strarray_add_uniq(&targets, "all");

  for (i = 0; i < NB_INSTALL_RULES; i++) {
    if (!make->install_rules[i].count)
      continue;
    strarray_add_uniq(&targets, "install");
    strarray_add(&targets, install_targets[i]);
  }
  if (make->clean_files.count)
    strarray_add(&targets, "clean");
  ...
  if (!targets.count && !make->clean_files.count)
    return;

  output_file_name = obj_dir_path(make, "Makefile");
  output_file = create_temp_file(output_file_name);

  output("# Auto-generated stub makefile; all rules forward to the top-level "
         "makefile\n\n");
...
static void output_silent_rules(void) {
  static const char *cmds[] = {"BISON", "BUILD", "CC",  "CCLD", "FLEX",
                               "GEN",   "LN",    "MSG", "SAST", "SED",
                               "TEST",  "WIDL",  "WMC", "WRC"};
  unsigned int i;
...
static void output_top_makefile(struct makefile *make) {
  ...
  while (fgets(buffer, sizeof(buffer), src_file) && !found) {
    if (fwrite(buffer, 1, strlen(buffer), output_file) != strlen(buffer))
      fatal_perror("write");
  ...
  output("Makefile: %s\n", makedep);
  output("depend: %s\n", makedep);
  output("\t%s%s%s\n", makedep, compile_commands_mode ? " -C" : "",
         silent_rules ? " -S" : "");
  strarray_add(&make->phony_targets, "depend");

  for (i = 0; i < subdirs.count; i++)
    output_sources(submakes[i]);
  output_sources(make);
...
static void output_dependencies(struct makefile *make) {
  ...
  if (make->obj_dir)
    create_dir(make->obj_dir);

  if (make == top_makefile)
    output_top_makefile(make);
  else
    output_stub_makefile(make);

  strarray_addall(&ignore_files, make->distclean_files);
  strarray_addall(&ignore_files, make->clean_files);
  if (make->testdll)
    output_testlist(make);
  if (make->obj_dir && !strcmp(make->obj_dir, "po"))
    output_linguas(make);
  if (!make->src_dir)
    output_gitignore(obj_dir_path(make, ".gitignore"), ignore_files);
...
static void load_sources(struct makefile *make) {
  ...
  if (make->module) {
    ...
    for (i = 0; i < NB_INSTALL_RULES; i++)
      if (make->install[i].count)
        break;
    if (i == NB_INSTALL_RULES) {
      if (make->importlib)
        strarray_add(&make->install[INSTALL_DEV], make->importlib);
      if (make->staticlib)
        strarray_add(&make->install[INSTALL_DEV], make->staticlib);
      else
        strarray_add(&make->install[INSTALL_LIB], make->module);
    }
  }
  ...
  for (i = 0; i < value.count; i++) {
    ...
    } else if (!strncmp(value.str[i], "-D", 2) ||
               !strncmp(value.str[i], "-U", 2))
      strarray_add_uniq(&make->define_args, value.str[i]);
  }
  strarray_addall(&make->define_args,
                  get_expanded_make_var_array(make, "EXTRADEFS"));
  strarray_addall_uniq(&make->unix_cflags,
                       get_expanded_make_var_array(make, "UNIX_CFLAGS"));

  strarray_add(&make->include_args, strmake("-I%s", obj_dir_path(make, "")));
  if (make->src_dir)
    strarray_add(&make->include_args, strmake("-I%s", make->src_dir));
  if (make->parent_dir)
    strarray_add(&make->include_args,
                 strmake("-I%s", src_dir_path(make, make->parent_dir)));
  ...
  for (i = 0; i < value.count; i++)
    add_src_file(make, value.str[i]);

  add_generated_sources(make);

  LIST_FOR_EACH_ENTRY(file, &make->includes, struct incl_file, entry)
  parse_file(make, file, 0);
  LIST_FOR_EACH_ENTRY(file, &make->sources, struct incl_file, entry)
  get_dependencies(file, file);

  for (i = 0; i < make->delayimports.count; i++)
    strarray_add_uniq(&delay_import_libs,
                      get_base_name(make->delayimports.str[i]));
}
...
static void parse_makeflags(const char *flags) {
  ...
  while (*p) {
    ...
    *var = 0;
    if (var > buffer)
      set_make_variable(&cmdline_vars, buffer);
  }
}
...
static int parse_option(const char *opt) {
  if (opt[0] != '-') {
    if (strchr(opt, '='))
      return set_make_variable(&cmdline_vars, opt);
    return 0;
  }
  switch (opt[1]) {
  case 'f':
    if (opt[2])
      output_makefile_name = opt + 2;
    break;
  case 'C':
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < NB_INSTALL_RULES; i++)
    top_install[i] = get_expanded_make_var_array(
        top_makefile, strmake("TOP_%s", install_variables[i]));
  ...
  if (root_src_dir && !strcmp(root_src_dir, "."))
    root_src_dir = NULL;
  if (tools_dir && !strcmp(tools_dir, "."))
    tools_dir = NULL;
  if (!exe_ext)
    exe_ext = "";
  if (!dll_ext[0])
  ...
  strarray_addall(&archs,
                  get_expanded_make_var_array(top_makefile, "PE_ARCHS"));
  ...
  arch_install_dirs[0] = unix_lib_supported
                             ? strmake("$(libdir)/wine/%s-unix/", archs.str[0])
                             : "$(libdir)/wine/";
  strip_progs[0] = "\"$(STRIP)\"";
  ...
  for (arch = 0; arch < archs.count; arch++) {
    extra_cflags[arch] =
        get_expanded_arch_var_array(top_makefile, "EXTRACFLAGS", arch);
    extra_cflags_extlib[arch] = remove_warning_flags(extra_cflags[arch]);
    disabled_dirs[arch] =
        get_expanded_arch_var_array(top_makefile, "DISABLED_SUBDIRS", arch);
    if (!is_multiarch(arch))
      continue;
    delay_load_flags[arch] =
        get_expanded_arch_var(top_makefile, "DELAYLOADFLAG", arch);
  ...
  for (i = 0; i < subdirs.count; i++)
    if (submakes[i] != include_makefile)
      load_sources(submakes[i]);

  output_dependencies(top_makefile);
  for (i = 0; i < subdirs.count; i++)
    output_dependencies(submakes[i]);

  if (compile_commands_mode)
